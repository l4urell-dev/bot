...
module.exports.run = async (bot, message, args) => {
    ...
    try {
        ...
        if (rand <= rand2) { embed.setDescription(msgs[2]); bot.lprofile.subtract(`coins_${message.author.id}_${message.guild.id}`, Math.floor(parseInt(args[0]))); return bot.send(embed); }
        if (rand > rand2) { embed.setColor('#10e250'); embed.setDescription(msgs[3]); bot.send(embed); }
        bot.lprofile.add(`coins_${message.author.id}_${message.guild.id}`, Math.floor(parseInt(args[0]) * 1.5) - Math.floor(parseInt(args[0])));
